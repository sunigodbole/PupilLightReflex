...
# Figure Individual Pupil Trajectories with a couple highlighted
...
post.user <- ggplot(right_0.post, aes(x=seconds, y = percent_change, group = subject_id))+
             ...
             stat_summary(aes(x = seconds, y = percent_change, group = user_cat), 
                          fun = "mean", linewidth = 1.1, geom = "line")+
             labs(y = "Percent change in pupil response", 
                  x = "Seconds from start of light test")+
             theme_bw()
...
```{r tab1_analyticN}
pt.analytic.df$postConsumpTimeToTest[pt.analytic.df$user_cat == "non-user"] <- 0

tab1 <- pt.analytic.df[,  c("user_cat", "demo_age", "demo_gender", "BMI", "thc.post", "postConsumpTimeToTest")]
...
pt.analytic.df$postConsumpTimeToTest[pt.analytic.df$user_cat == "non-user"] <- NA

boxplot(pt.analytic.df$postConsumpTimeToTest)
hist(pt.analytic.df$postConsumpTimeToTest)
summary(pt.analytic.df$postConsumpTimeToTest)
```

```{r saveTable1}
table1.df %>% 
  as_gt() %>%
  gt::gtsave(filename = "../results/tabl1.png")
```
...
pt.scores <- merge(post_right_scores, pt.analytic.df, 
                   by = c("subject_id"), 
                   all.x = T)
...
pt.scalar.roc <- roc(response = pt.scores$smoker, predictor = pt.scores$pred, ci=TRUE)
# pt.scalar.roc
post.auc.scalar <- pt.scalar.roc$auc
post.auc.lci.scalar <- as.numeric(pt.scalar.roc$ci)[1]
post.auc.uci.scalar <- as.numeric(pt.scalar.roc$ci)[3]
...
smk.df <- pt.analytic.df[, c("subject_id", "user_cat", "female", "BMI")]
...
auc.df <- data.frame("Label" = c("Post AUC Scalar Feat", "Post SoFR AUC"), 
                     ...
                     "lci" = c(post.auc.lci.scalar, pt.sofr.auc.lci), 
                     "uci" = c(post.auc.uci.scalar, pt.sofr.auc.uci),
                     stringsAsFactors = F)
...
smk_fglm_cv <- gam(smoker ~ female + BMI +
...
# Sex and gender added as functional coefficients
## -- Not sure if this is the right specification
smk_fglm_fcv <- gam(smoker ~ s(smat, by=zlmat, bs = "cr", k = 30)+
                      s(smat, by= femaleMat, bs = "cr", k = 30)+
                      s(smat, by= bmiMat, bs = "cr", k = 30),
                    data=smk.df, 
                   method = "REML", family = binomial)

anova(smk_fglm_fcv, smk_fglm_ps2, test = "Chisq")
```
...
```{r roc_sofr_plots, fig.height= 4, fig.width=8, warning=FALSE, dev='jpeg'}
...
```{r fosr_model_plots, fig.height=8, fig.width=15, dev='jpeg'}
...
fosr_sex_gen_sm.fri <- mgcv::bam(percent_change ~
                                s(sind, by=female, k=30, bs = "cr") + 
                                s(sind, by=BMI, k=30, bs="cr") +  
                            s(sind, k=30, bs="cr") + 
                            s(sind, by=occasional, k=30, bs = "cr") + 
                            s(sind, by=daily, k=30, bs = "cr") +
                            s(subject_id, by = Phi1, bs="re") + s(subject_id, by = Phi2, bs="re")+
...
anova(fosr_sex_gen_sm.fri, post_gam.fri, test = "Chisq")
```


```{r plot_sex_coef}
post_gam.coef <- fosr_sex_gen_sm.fri$coefficients
post_gam.cov <- vcov.gam(fosr_sex_gen_sm.fri)

df_pred_male <- data.frame("sind" = unique(right_0.gam.df$sind), 
                          "female" = 0, 
                          "BMI" = 0,
...
lpmat_male <- predict(fosr_sex_gen_sm.fri, newdata=df_pred_male, se.fit=TRUE, type = "lpmatrix")


df_pred_female <- data.frame("sind" = unique(right_0.gam.df$sind),
                             "female" = 1, 
                             "BMI" = 0,
                             "occasional" = 0, "daily" = 0, 
                            "Phi1" = 0, "Phi2" = 0, "Phi3" = 0, 
...
pred_female <- predict(fosr_sex_gen_sm.fri, newdata=df_pred_female, se.fit=TRUE, type = "terms")

pred_sex_coef <- data.frame(seconds = seq(0, 400)/30,
                           f0_hat = lpmat_male %*% post_gam.coef, 
                           f0_se = sqrt(diag(lpmat_male %*% post_gam.cov %*% t(lpmat_male))), 
                           f1_hat = pred_female$fit[, 1], 
                           f1_se = pred_female$se.fit[, 1])

post_sex_plt <- ggplot(data = pred_sex_coef, aes(x=seconds, y = f1_hat))+
                geom_line()+
                geom_line(aes(x = seconds, y = f1_hat + 2*f1_se), linetype = "longdash")+
                geom_line(aes(x = seconds, y = f1_hat - 2*f1_se), linetype = "longdash")+
                scale_x_continuous(expand = c(0, 0), limits = c(0,10))+
                scale_y_continuous(limits = c(-6, 6))+
                labs(title = "Difference in Average Percent Change Between Males and Females", 
                     y = "Percent Change", 
                     x = "Seconds from start of light test")+
                theme_bw()
...
df_pred_bmi <- data.frame("sind" = unique(right_0.gam.df$sind),
                             "female" = 0, 
                             "BMI" = 1,
                             "occasional" = 0, "daily" = 0, 
                            "Phi1" = 0, "Phi2" = 0, "Phi3" = 0, 
...
pred_bmi <- predict(fosr_sex_gen_sm.fri, newdata=df_pred_bmi, se.fit=TRUE, type = "terms")

pred_bmi_coef <- data.frame(seconds = seq(0, 400)/30,
                           f1_hat = pred_bmi$fit[, 2], 
                           f1_se = pred_bmi$se.fit[, 2])

post_bmi_plt <- ggplot(data = pred_sex_coef, aes(x=seconds, y = f1_hat))+
                geom_line()+
                geom_line(aes(x = seconds, y = f1_hat + 2*f1_se), linetype = "longdash")+
                geom_line(aes(x = seconds, y = f1_hat - 2*f1_se), linetype = "longdash")+
                scale_x_continuous(expand = c(0, 0), limits = c(0,10))+
                scale_y_continuous(limits = c(-6, 6))+
                labs(title = "Difference in Average Percent Change for one unit change in BMI", 
...
```{r PCTT_hist_response, fig.height= 4, fig.width=8, warning=FALSE, dev='jpeg'}
