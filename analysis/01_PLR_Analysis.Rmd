...
smk_fglm_ps3 <- gam(smoker ~ s(smat, by=zlmat, bs = "cr", k = 30), data=smk.df, 
                   method = "REML", family = binomial)
summary(smk_fglm_ps3)
...
smk_fglm_cv <- gam(smoker ~ female + BMI_c +
                   ...
                   method = "REML", family = binomial)
summary(smk_fglm_cv)
anova(smk_fglm_cv, smk_fglm_ps3, test = "Chisq")
```
...
df_sofr <- data.frame("smat" = seq(0, 1, length.out = ncol(smk.df$smat)),
                      "zlmat" = 1)
sofr_pred<- predict(smk_fglm_ps3, newdata = df_sofr, se.fit = TRUE, type = "iterms")
...
```{r fosr_concurv}
concurvity(post_gam.fri, full = FALSE)
```
