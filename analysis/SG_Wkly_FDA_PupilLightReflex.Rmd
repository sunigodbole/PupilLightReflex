...
```{r eyeCorr}
right.names <- names(right_0.post.w)
right.names <- c(right.names[1:3], paste0("R_", right.names[4:length(right.names)]))
names(right_0.post.w) <- right.names

left.names <- names(left_0.post.w)
left.names <- c(left.names[1:3], paste0("L_", left.names[4:length(left.names)]))
names(left_0.post.w) <- left.names


botheyes <- merge(right_0.post.w, left_0.post.w, 
