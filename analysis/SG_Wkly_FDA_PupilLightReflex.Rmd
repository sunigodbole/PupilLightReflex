...
# To Do: 
1. Whiteboarding coef in terms of model (Mtg: 10/6/2022 )
2. Construct $\beta$ for trajectory of occasional user (mean structure)
3. Construct CI by hand
4. Add random effect of subject to gam/bam model using slides 6&7 in lecture 11 of FDA class (2-step approach)
5. Review data for pffr fxn -- NaN in output might indicate missing subjects
6. Try different number of basis functions start with 15 go to 30
7. Write down mathematical equations for foSR 
  + Try to construct point estimate  & CI for occasional user
8. Write down mathematical interpretation of the plots. 


# Task Worked On From Last Week:
1.Truncate data to non-missing samples around frame 400
2. Compare scalar vs functional AUC curves (need scalar features from Ben)
3. Fit foSR with gam/bam
6. Function on Scalar Regression (use class notes)
...
## Within-Person Difference: Logistic Regression models to predict Smoker vs non-smoker using Within Person Difference data. plot AUC
...
## Run logistic regression with same sample and same # of PCs. 
...
&= \sum_{{k_0}=1}^{K_0} \xi_{k_0}\phi_{k_0}(t_i) + \sum_{k=1}^K \xi_k\phi_k(t_i)*I(user = occasional) + \sum_{k=1}^K \xi_k\phi_k(t_i)*I(user = daily) + \sum_{k=1}^K \xi_{ik}\phi_k(t_i) + \epsilon_i(t)\\
\end{aligned}
$$
...
post_gam.resid <- cbind(right_0.gam.df[, c("subject_id", "frame")], m.post_gam$residuals)
names(post_gam.resid) <- c("subject_id", "frame", "resid")
resid_mat <- reshape(post_gam.resid, 
                     timevar = "frame", 
                     idvar = "subject_id", 
                     direction = "wide")
...
```
### Plotting trajectory of occasional user
```{r plot_occ}
post_gam.coef <- post_gam.fri$coefficients
post_gam.cov <- vcov.gam(post_gam.fri)
df_pred_rfi <- data.frame("sind" = unique(right_0.gam.df$sind), "occasional" = 1, "daily" = 0, 
                            "Phi1" = 0, "Phi2" = 0, "Phi3" = 0, 
                            "Phi4" = 0, 
                          "subject_id" = right_0.gam.df$subject_id[1])

yhat_rfi <- predict(post_gam.fri, newdata=df_pred_rfi, se.fit=TRUE, type = "response")
fhat_rfi <- predict(post_gam.fri, newdata=df_pred_rfi, se.fit=TRUE, type = "terms")

# design.mat <- PredictMat(post_gam.fri, df_pred_rfi)
plot.yhat.df <- data.frame("frame" = 0:400, 
                            "yhat" = yhat_rfi$fit, 
                            "yhat.se" = yhat_rfi$se.fit)

plot.f.df <- data.frame("frame" = 0:400, 
                        "fhat_rfi_f0" = fhat_rfi$fit[, 1] + post_gam.fri$coefficients[1], 
                        "fhat_rfi_f0.se" = fhat_rfi$se.fit[, 1], 
                        "fhat_rfi_f1" = fhat_rfi$fit[, 2], 
                        "fhat_rfi_f1.se" = fhat_rfi$se.fit[, 2])

ggplot(data = plot.yhat.df, aes(x = frame, y = yhat))+
  geom_line()+
  geom_line(aes(x=frame, y = yhat + 2*yhat.se), linetype = "longdash")+
  geom_line(aes(x=frame, y = yhat - 2*yhat.se), linetype = "longdash")+
  #geom_line(data = plot.f.df, aes(x=frame, y = fhat_rfi_f0+fhat_rfi_f1) ,col = "blue")+
  theme_bw()
```
...
## Create a matrix of residuals
wi_gam.resid <- cbind(right_0.gam.df[, c("subject_id", "frame")], m.post_gam$residuals)
names(wi_gam.resid) <- c("subject_id", "frame", "resid")
resid_mat <- reshape(wi_gam.resid,
                     timevar = "frame",
                     idvar = "subject_id",
                     direction = "wide")
rownames(resid_mat) <- resid_mat$subject_id
...
df_pt_pred_rfi <- data.frame("sind" = unique(right_0.pt.gam.df$sind), "occasional" = 1, "daily" = 0, 
                            "Phi1" = 0, "Phi2" = 0, "Phi3" = 0, 
                            "Phi4" = 0, "Phi5" = 0, "Phi6" = 0, 
                          "subject_id" = right_0.pt.gam.df$subject_id[1])

yhat_pt_rfi <- predict(wi_gam.fri, newdata=df_pt_pred_rfi, se.fit=TRUE, type = "response")
fhat_pt_rfi <- predict(wi_gam.fri, newdata=df_pt_pred_rfi, se.fit=TRUE, type = "terms")

# design.mat <- PredictMat(post_gam.fri, df_pred_rfi)
plot.yhat_pt.df <- data.frame("frame" = 0:400, 
                            "yhat" = yhat_pt_rfi$fit, 
                            "yhat.se" = yhat_pt_rfi$se.fit)

plot.f_pt.df <- data.frame("frame" = 0:400, 
                        "fhat_rfi_f0" = fhat_pt_rfi$fit[, 1] + wi_gam.fri$coefficients[1], 
                        "fhat_rfi_f0.se" = fhat_pt_rfi$se.fit[, 1], 
                        "fhat_rfi_f1" = fhat_pt_rfi$fit[, 2], 
                        "fhat_rfi_f1.se" = fhat_pt_rfi$se.fit[, 2])

ggplot(data = plot.yhat_pt.df, aes(x = frame, y = yhat))+
  geom_line()+
  geom_line(aes(x=frame, y = yhat + 2*yhat.se), linetype = "longdash")+
  geom_line(aes(x=frame, y = yhat - 2*yhat.se), linetype = "longdash")+
  # geom_line(data = plot.f_pt.df, aes(x=frame, y = fhat_rfi_f0+fhat_rfi_f1) ,col = "blue")+
  theme_bw()
```
