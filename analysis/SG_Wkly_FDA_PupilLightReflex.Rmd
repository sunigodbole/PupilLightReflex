...
3. Try different number of basis functions start with 15 go to 30
4. Write down mathematical equations for foSR 
  + Try to construct point estimate  & CI for occasional user
5. Write down mathematical interpretation of the plots. 
...
```{r coef_effect_plots}
pred_f0 <- predict(post_gam.fri, newdata=df_pred_non, se.fit=TRUE, type = "terms")
...
pred_coef_df <- data.frame(frame = seq(0, 400),
                           f0_hat = pred_f0$fit[, 1], 
                           f0_se = pred_f0$se.fit[, 1], 
                           f1_hat = pred_f1$fit[, 2], 
                           f1_se = pred_f1$se.fit[, 2], 
                           f2_hat = pred_f2$fit[, 3], 
                           f2_se = pred_f2$se.fit[, 3])

ggplot(data = pred_coef_df, aes(x=frame, y = f0_hat+post_gam.fri$coefficients[1]))+
  geom_line()+
  geom_line(aes(x = frame, y = f0_hat+post_gam.fri$coefficients[1] + 2*f0_se), linetype = "longdash")+
  geom_line(aes(x = frame, y = f0_hat+post_gam.fri$coefficients[1] - 2*f0_se), linetype = "longdash")+
  labs(title = "Average Response of a Non-user", ylab = f0_hat)+
  theme_bw()

ggplot(data = pred_coef_df, aes(x=frame, y = f1_hat))+
  geom_line()+
  geom_line(aes(x = frame, y = f1_hat + 2*f1_se), linetype = "longdash")+
  geom_line(aes(x = frame, y = f1_hat - 2*f1_se), linetype = "longdash")+
  labs(title = "Difference in Response between Occasional and Non-user")+
  theme_bw()

ggplot(data = pred_coef_df, aes(x=frame, y = f2_hat))+
  geom_line()+
  geom_line(aes(x = frame, y = f2_hat + 2*f2_se), linetype = "longdash")+
  geom_line(aes(x = frame, y = f2_hat - 2*f2_se), linetype = "longdash")+
  labs(title = "Difference in Response between Daily and Non-user")+
  theme_bw()
```
