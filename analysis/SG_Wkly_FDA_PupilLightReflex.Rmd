...
3. Regression using VAS score to define "high" vs "not" in all smokers
    + Scatterplot of VAS vs point of minimal constriction for all smokers 
4. Time from Consumption to test -- WPD add FoSR
...
1. Impute with average for frames with missing values between known values
2. Scalar on Function Regression
...
3. Regression using VAS score to define "high" vs "not" in occasional smokers
...
pre_vas_minConstrict <- ggplot(data = AllSmokers, 
                             ...
                             stat_cor(label.y = 0, col = "black")+
                             stat_regline_equation(aes(col = NULL), label.y = -3, col = "black")+
                             # stat_poly_line(aes(col = NULL), se = FALSE) +
...
post_vas_minConstrict <- ggplot(data = AllSmokers, 
                             ...
                             geom_smooth(aes(color = NULL),se=FALSE, method = lm, formula = y ~ x)+
                             theme_bw()
...
pre_vas_minConstrict_occ <- ggplot(data = pt.analytic.df[pt.analytic.df$user_cat == "occasional", ], aes(x = vas1_high_score, y = min_constriction.pre2))+
                             geom_point()+
                             geom_vline(xintercept = 46,color = "darkred")+
                             labs(title = "POST VAS Feel High Score vs PRE minimal Constriction \nin Occasional Users")+
                             theme_bw()
...
post_vas_minConstrict_occ <- ggplot(data = pt.analytic.df[pt.analytic.df$user_cat == "occasional", ], aes(x = vas1_high_score, y = min_constriction.post))+
                             geom_point()+
                             geom_vline(xintercept = 46,color = "darkred")+
                             labs(title = "POST VAS Feel High Score vs POST minimal Constriction \nin Occasional Users")+
                             theme_bw()

chg_vas_minConstrict_occ <- ggplot(data = pt.analytic.df[pt.analytic.df$user_cat == "occasional", ], aes(x = vas1_high_score, y = min_constriction_chg))+
                            geom_point()+
                            geom_vline(xintercept = 46,color = "darkred")+
                            labs(title = "POST VAS Feel High Score vs Change in minimal Constriction (post-pre) \nin Occasional Users")+
                            theme_bw()

jpeg(file.path(res_folder, "Scatterplot_VAS_MinConstriction.jpg"),
     height = 6, width = 23, res = 300, units = "in")
...
## Function on Scalar Regression
### Post -- Interaction with Post Consumption Time to Test (PCTT)
$$ 
\begin{aligned}
...
```{r coef_effect_plots_intPCTT, eval=F}
pred_f1 <- predict(post_gam.fri, newdata=df_pred_occ, se.fit=TRUE, type = "terms")
pred_f2 <- predict(post_gam.fri, newdata=df_pred_dly, se.fit=TRUE, type = "terms")
