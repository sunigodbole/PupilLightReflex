...
pred_f0 <- predict(wi_gam.fri, newdata=df_pred_non, se.fit=TRUE, type = "terms")
...
ggplot(data = pred_coef_df, aes(x=frame, y = f0_hat + wi_gam.fri$coefficients[1]))+
  geom_line()+
  geom_line(aes(x = frame, y = f0_hat+ wi_gam.fri$coefficients[1] + 2*f0_se), linetype = "longdash")+
  geom_line(aes(x = frame, y = f0_hat+ wi_gam.fri$coefficients[1] - 2*f0_se), linetype = "longdash")+
  labs(title = "Average Response of a Non-user", ylab = "f0_hat")+
  theme_bw()
