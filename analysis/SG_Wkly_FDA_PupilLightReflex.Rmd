...
3. Time from Consumption to test -- WPD add FoSR
...
1. Scalar on Function Regression - Plot
...
2. Try FoSR model with non-linear effect of PCTT. 
3. Create a document with results to send to the collaborators
...
```{r tab1_analyticN}
# Demographics Table by User Category
# label(pt.df$demo_age) <- "Age (years)"
# label(pt.df$demo_gender) <- "Sex"
# label(pt.df$BMI) <- "BMI (kg/m^2)"
# label(pt.df$postConsumpTimeToTest) <- "Time Delay After Marijuana Consumption (mins)"
# label(pt.df$thc.post) <- "Blood THC (post consumption)"
# 
# table1(~demo_age + demo_gender + BMI + postConsumpTimeToTest + thc.post|user_cat, 
...
table1.df <- tbl_summary(tab1,
                         ...
                         label = list(
                           ...
                           thc.post ~ "THC, post consumption (mg/dL)", 
                           postConsumpTimeToTest ~ "Time Delay post Consumption (mins)"
                         ),
  ...
  # add_n() %>%
  modify_spanning_header(c("stat_1", "stat_2", "stat_3") ~ "**Marijuana Use Frequency**") %>%
  bold_labels()
table1.df
...
ggplot(data = plot.sofr, aes(x=frame/30, y = exp(f0)))+
  geom_line()+
  geom_line(aes(x=frame/30, y = exp(f0-2*f0.se)), linetype = 'longdash')+
  geom_line(aes(x=frame/30, y = exp(f0+2*f0.se)), linetype = 'longdash')+
...
## Start and End of first region of sig diff.
start1 <- min(plot.sofr$sec[plot.sofr$lci > 1 & plot.sofr$uci > 1])
end1 <- max(plot.sofr$sec[plot.sofr$lci > 1 & plot.sofr$uci > 1])


or1 <- max(plot.sofr$OR[plot.sofr$sec > start1 & plot.sofr$sec < end1])
peaktime1 <- plot.sofr$sec[plot.sofr$OR == or1]
lci1 <- round(plot.sofr$lci[plot.sofr$OR == or1], 2)
uci1 <- round(plot.sofr$uci[plot.sofr$OR == or1], 2)
or1 <- round(or1, 2)


## Start and End of Second region of sig diff
start2 <- min(plot.sofr$frame[plot.sofr$lci < 1 & plot.sofr$uci < 1])
end2 <- max(plot.sofr$frame[plot.sofr$lci < 1 & plot.sofr$uci < 1])
...
legend(0.5, 0.35, legend = c(paste0("Summary Features AUC: ",round(post.auc.scalar, 3)),
...
post_userProfile.se <- ggplot(data = pred_df_user, aes(x = seconds, y = mean, group = user, col = user))+
                    geom_line()+
...
post_occ_plt <- ggplot(data = pred_coef_df, aes(x=seconds, y = f1_hat))+
                ...
                labs(title = "Occasional v Non-user", 
                     y= "")+
                theme_bw()+
...
post_dlyocc_plt <- ggplot(data = f2_f1_diff_df, aes(x=seconds, y = f2f1_diff))+
                   ...
                   geom_line(aes(x = seconds, y = f2f1_diff - 2*f2f1_se), linetype = "longdash")+
                   geom_line(aes(x = seconds, y = 0), col = "darkred")+
                   labs(title = "Daily vs Occasional User", 
                        y = "")+
                   theme_bw()+
                   scale_x_continuous(expand = c(0, 0))+
                   theme(rect = element_rect(fill = "transparent"))
...
grid.arrange(ylab, posval, negval, posval_do, negval_do, 
             post_occ_plt, post_dly_plt, post_dlyocc_plt, blank, 
                                 ncol = 4, nrow = 4, 
                                 layout_matrix = rbind(c(1, 2, rep(6, 8), rep(7, 8)),
                                                       c(1, 3, rep(6, 8), rep(7, 8)),
                                                       c(1, 4, rep(8, 8), rep(9, 8)),
                                                       c(1, 5, rep(8, 8), rep(9, 8))))
...
post_smkProfile.se <- ggplot(data = pred_df_smk, aes(x = seconds, y = mean, group = user, col = user))+
                      geom_line()+
                      geom_line(aes(x=seconds, y = mean + 2*se, group = user, col = user),
                                linetype = "longdash")+
                      geom_line(aes(x=seconds, y = mean - 2*se, group = user, col = user),
...
post_allProfile <- ggplot(data = pred_df_user, aes(x = seconds, y = mean, group = user, col = user))+
                   geom_line()+
                   geom_line(data = pred_df_smk[pred_df_smk$user == "smoker", ], 
                             aes(x = seconds, y = mean), 
                             linetype = "longdash")+
                   labs(y = "Percent Change in Pupil Response", 
                        x = "Seconds are Start of Light Test", 
                        colour = "Cannabis Use")+
                   theme_bw()
...
ggplot(data= pt.analytic.df[pt.analytic.df$user_cat != "non-user", ], 
       aes(x = postConsumpTimeToTest))+
  geom_histogram(binwidth = 1)+
  geom_vline(xintercept = mean(pt.analytic.df$postConsumpTimeToTest, na.rm = T), 
             col = "darkred")+
  scale_y_continuous(expand = c(0,0))+
  scale_x_continuous(breaks = round(seq(53, 84, length.out = 5), 1),
                     labels = as.character(round(seq(53, 84, length.out = 5), 1)))+
  labs(x= "Post Consumption Time to Test", 
       caption = "Red line at mean value")+
  theme_bw()
```
...
Histogram of post consumption time to test
```{r Hist_PCTT}
ggplot(pt.analytic.df, aes(x = postConsumpTimeToTest))+
  geom_histogram()
...
pred_df <- data.frame(seconds = rep(seq(0, 400), 4)/30,
                      user = c(rep("non-smoker", 401), 
                               rep("smoker (65 min)", 401), 
                               rep("smoker (60 min)", 401), 
                               rep("smoker (70 min)", 401)
                               ), 
...
post_smokerTraj
```
### Trajectories for smokers and non-smokers with non-linear effect of time and PCTT

```{r post_gam_intPCTT_smokers_NL}
pt.analytic.df$postConsumpTimeToTest_c[is.na(pt.analytic.df$postConsumpTimeToTest_c)] <- 0
...
m.post_intPCTT_smoke_gam_NL <- mgcv::bam(percent_change ~ s(sind, k=30, bs="cr") +
                                  s(sind, by=smoker, k=30, bs = "cr") +
                                  ti(sind, postConsumpTimeToTest_c,
                                     by = smoker, 
                                     k=c(30, 8), bs=c("cr", "cr"),
                                     mc=c(FALSE, TRUE)),
                                data = post_intPCTT_smoke.gam.df, method = "fREML", 
                                discrete = TRUE)

summary(m.post_intPCTT_smoke_gam_NL)

## Create a matrix of residuals
post_intPCTT_smoke_gam.resid <- cbind(post_intPCTT_smoke.gam.df[!(is.na(post_intPCTT_smoke.gam.df$percent_change)), 
                                       c("subject_id", "frame")],
                                m.post_intPCTT_smoke_gam_NL$residuals)
names(post_intPCTT_smoke_gam.resid) <- c("subject_id", "frame", "resid")
# post_gam.resid$frame_char <- str_pad(post_gam.resid$frame, width = 3, side = "left", pad = "0")
...
sind <- seq(0, 1, length.out = 401)
PCTT <- c(55,60,65,70) - round(mean(pt.analytic.df$postConsumpTimeToTest, na.rm = T), 2)
pred.df <- expand.grid(sind=sind, PCTT = PCTT)
pred.df$smoker <- 1

ns.df <- data.frame("sind" = seq(0, 1, length.out = 401), 
                    "PCTT" = 0, 
                    "smoker" = 0)

pred.df <- rbind(pred.df, ns.df)

pred.df$Phi1 <- 0
pred.df$Phi2 <- 0
pred.df$Phi3 <- 0
pred.df$Phi4 <- 0
pred.df$subject_id <- pt.analytic.df$subject_id[1]

names(pred.df) <- c("sind", "postConsumpTimeToTest_c", "smoker", "Phi1", "Phi2", "Phi3", "Phi4", "subject_id")

all.df <- predict(post_intPCTT_smoke.gam.fri, newdata = pred.df, se.fit=TRUE, type = "lpmatrix")

pred_df <- data.frame(seconds = rep(seq(0, 400), 5)/30,
                      user = c(rep("smoker-55", 401), 
                               rep("smoker-60", 401), 
                               rep("smoker-65", 401), 
                               rep("smoker-70", 401), 
                               rep("non-smoker", 401)
                               ), 
                      mean = all.df %*% post_intPCTT_smoke_gam.coef, 
                      se = sqrt(diag(all.df %*% post_intPCTT_smoke_gam.cov %*% t(all.df))), 
                      stringsAsFactors = F)

ggplot(data = pred_df, aes(x = seconds, y = mean, group = user, col = user))+
                    geom_line()+
                    labs(y = "Percent Change")+
                    theme_bw()
```
...
### ONLY smokers -- non-linear effect of PCTT
```{r post_gam_intPCTT_ONLYsmokersINT}
### Subset to only smokers and remove previously calculated "Phi" values
post_intPCTT_smoke.gam.df <- post_intPCTT_smoke.gam.df[post_intPCTT_smoke.gam.df$smoker == 1, ] 
keep.vars <- names(post_intPCTT_smoke.gam.df)[grepl("Phi", names(post_intPCTT_smoke.gam.df)) == FALSE]
post_intPCTT_smoke.gam.df <- post_intPCTT_smoke.gam.df[, keep.vars]
...
m.post_intPCTT_smoke_gamInt <- mgcv::bam(percent_change ~ s(sind, bs = "cr", k = 30)+
                                        ti(sind, postConsumpTimeToTest_c, 
                                                          k=c(30, 5), bs=c("cr", "cr"),
                                                          mc=c(FALSE, TRUE)),
                                data = post_intPCTT_smoke.gam.df, method = "fREML", discrete = TRUE)
summary(m.post_intPCTT_smoke_gamInt)

## Create a matrix of residuals
post_intPCTT_smoke_gam.resid <- cbind(post_intPCTT_smoke.gam.df[!(is.na(post_intPCTT_smoke.gam.df$percent_change)), 
                                       c("subject_id", "frame")],
                                m.post_intPCTT_smoke_gamInt$residuals)
names(post_intPCTT_smoke_gam.resid) <- c("subject_id", "frame", "resid")
# post_gam.resid$frame_char <- str_pad(post_gam.resid$frame, width = 3, side = "left", pad = "0")
...
post_intPCTT_smoke.gam.fri <- mgcv::bam(percent_change ~ 
                            s(sind, k=30, bs="cr") +
                            s(sind, by=postConsumpTimeToTest_c, k=5, bs = "cr") +
                            ti(sind, postConsumpTimeToTest_c,
                               k=c(30, 5), bs=c("cr", "cr"), mc=c(FALSE, TRUE))+
                            s(subject_id, by = Phi1, bs="re") + s(subject_id, by = Phi2, bs="re")+
                            s(subject_id, by = Phi3, bs="re") + s(subject_id, by = Phi4, bs="re"), 
                          method = "fREML", data = post_intPCTT_smoke.gam.df, discrete = TRUE)
...
### Try to plot Additive Functional Regression
```{r plot_addFunc}
cal_grid <- function(grid_length = 100, lo_x = 1, high_x, range_s = 1){
    tind_pred <- seq(0, range_s, length.out = grid_length)
    xind_pred <- seq(lo_x, high_x, length.out = grid_length)
    grid_coef <- data.frame(t_int = rep(tind_pred, grid_length),
                            func_cov = rep(xind_pred, each = grid_length))
    return(grid_coef)
}

grid_coef <- cal_grid(grid_length = 100, lo_x = min(pt.analytic.df$postConsumpTimeToTest_c, na.rm = T), 
                      high_x = max(pt.analytic.df$postConsumpTimeToTest_c, na.rm = T), 
                      range_s = 1)
grid_coef$Phi1 <- 0
grid_coef$Phi2 <- 0
grid_coef$Phi3 <- 0
...
names(grid_coef) <- c("sind", "postConsumpTimeToTest_c", "Phi1", "Phi2", "Phi3", "Phi4")
grid_coef$subject_id <- pt.analytic.df$subject_id[1]

pred_coef <- predict(post_intPCTT_smoke.gam.fri, newdata = grid_coef, type = "terms")


```

### Exploring quadrature weights applied to original trajectories
```{r quadwt}
Zraw <- I(as.matrix(right_0.post.w[, pctChg_names]))
ncols = ncol(Zraw)
lmat <- I(matrix(1/ncols, nrow(Zraw), ncols))
zlmat <- I(lmat * Zraw)


zlmat <- as.data.frame(cbind(zlmat))
var.names <- names(zlmat)
zlmat$pt_id <- rownames(zlmat)

zlmat.l <- reshape(zlmat, 
                   varying = var.names, 
                   v.names = "pctChg", 
                   timevar = "frame", 
                   times = seq(0,400), 
...
zlmat.l <- zlmat.l[order(zlmat.l$pt_id, zlmat.l$frame), ]
zlmat.l$id <- NULL

ggplot(data = zlmat.l, aes(x=frame, y = pctChg, group = pt_id))+
  geom_line()+
  theme_bw()

test <- aggregate(frame ~ pt_id, data = zlmat.l[!(is.na(zlmat.l$pctChg)), ], FUN = "max")

ggplot(data = right_0.post, aes(x=frame, y= percent_change, group = subject_id))+
  geom_line()+
  theme_bw()

ggplot(data = zlmat.l[zlmat.l$pt_id == "001-011", ], aes(x=frame, y = pctChg, group = pt_id))+
  geom_line()+
  theme_bw()

ggplot(data = right_0.post[right_0.post$subject_id == "001-011", ], aes(x=frame, y= percent_change, 
                                                                        group = subject_id))+
  geom_line()+
  theme_bw()
```
