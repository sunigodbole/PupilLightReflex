...
plot.sofr <- data.frame("frame" = 0:375,
                            "f0" = sofr_pred$fit[, 1],
...
ggplot(data = plot.sofr, aes(x=frame/300, y = f0))+
  geom_line()+
  geom_line(aes(x=frame, y = f0-2*f0.se), linetype = 'longdash')+
  geom_line(aes(x=frame, y = f0+2*f0.se), linetype = 'longdash')+
  labs(title = "Odd Ratio (?) of being a smoker vs non-smoker over the test duration")+
  theme_bw()
...
df_pred_occInt <- data.frame("sind" = unique(post_intPCTT.gam.df$sind), "occasional" = 0, "daily" = 0, "occ_PCTT" = 10, "dly_PCTT" = 0,
                            "Phi1" = 0, "Phi2" = 0, "Phi3" = 0, "Phi4" = 0, 
                          "subject_id" = post_intPCTT.gam.df$subject_id[1])
...
pred_coef_df <- data.frame(seconds = seq(0, 400)/30,
                           # f0_hat = lpmat_non %*% post_gam.coef, 
                           # f0_se = sqrt(diag(lpmat_non %*% post_gam.cov %*% t(lpmat_non))), 
                           f1_hat = pred_f1$fit[, 1], 
                           f1_se = pred_f1$se.fit[, 1]
                           # ,
                           # f2_hat = pred_f2$fit[, 3],
                           # f2_se = pred_f2$se.fit[, 3]
                           )
...
post_intPCTT_smoke.gam.df <- post_intPCTT_smoke.gam.df[order(post_intPCTT.gam.df$subject_id,
                                                             post_intPCTT.gam.df$frame), ]
