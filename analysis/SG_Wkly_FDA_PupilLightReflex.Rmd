...
```{r tab1}
# Demographics Table by User Category
table1(~ demo_age + demo_weight+ demo_height+BMI + demo_gender + thc_chg + postConsumpTimeToTest + Post_PreTime + min_constriction_chg + AUC_chg + duration_chg + slope_chg + avg_end_percent_chg + vas|user_cat, 
       data = pt.df)
```
...
left_0.df$seconds <- left_0.df$frame/30
...
right_0.df$seconds <- right_0.df$frame/30
...
postConsump.cor <- cor(pt.analytic.df$postConsumpTimeToTest, pt.analytic.df[, c("demo_age", "demo_weight", "demo_height", "BMI",
                                                             "thc.post", "min_constriction.post", "slope.post", "AUC.post",
                                                             "thc_chg", "min_constriction_chg", "AUC_chg",
                                                             "wpd.PC1", "wpd.PC2", "wpd.PC3", "wpd.PC4", "wpd.PC5", "wpd.PC6", 
                                                             "post.PC1", "post.PC2", "post.PC3", "post.PC4",
                                                             "vas", "Post_PreTime")], use = "pairwise.complete.obs")

postConsump.rcorr <- rcorr(as.matrix(pt.analytic.df[, c("postConsumpTimeToTest", "demo_age", "demo_weight", "demo_height", "BMI",
                                                        "thc.post", "min_constriction.post", "slope.post", "AUC.post",
                                                        "thc_chg", "min_constriction_chg", "AUC_chg",
                                                        "wpd.PC1", "wpd.PC2", "wpd.PC3", "wpd.PC4", "wpd.PC5", "wpd.PC6", 
                                                        "post.PC1", "post.PC2", "post.PC3", "post.PC4",
...
sind <- seq(0, 1, len = 401)
smat <- matrix(sind, nrow(smk.df), 401, byrow = T)

smk.df$Zsm <- I(Zsm)
smk.df$Zraw <- I(right_0.post.w)
smk.df$smat <- I(smat)
smk.df$lmat <- I(matrix(1/401, nrow(smk.df), 401))
smk.df$zlmat <- I(smk.df$lmat * smk.df$Zsm)

smk_fglm_ps <- gam(smoker ~ s(smat, by=zlmat, bs = "cr", k = 30), data=smk.df, 
                   method = "REML", family = binomial)
summary(smk_fglm_ps)

roc_in_sample <- roc(smk.df$smoker, smk_fglm_ps$fitted.values)
auc(roc_in_sample)
```
...
```{r vas_explore}
hist(pt.analytic.df$vas[pt.analytic.df$user_cat == "daily"], breaks = 10)
boxplot(vas ~ user_cat, data = pt.analytic.df)
summary(pt.analytic.df$vas[pt.analytic.df$user_cat == "daily"])
summary(pt.analytic.df$vas[pt.analytic.df$user_cat == "occasional"])
sum(pt.analytic.df$vas[pt.analytic.df$user_cat == "occasional"] >= 60, na.rm = T)
sum(pt.analytic.df$vas[pt.analytic.df$user_cat == "occasional"] < 60, na.rm = T)

pt.analytic.df$vas_high <- NA
pt.analytic.df$vas_high[pt.analytic.df$vas >= 60 & pt.analytic.df$user_cat == "occasional"] <- 1
pt.analytic.df$vas_high[pt.analytic.df$vas < 60 & pt.analytic.df$user_cat == "occasional"] <- 0
```
...
m.post_vas <- mgcv::gam(percent_change ~ s(sind, k=30, bs="cr") + s(sind, by=vas_high, k=30, bs = "cr"), 
                  data = right_0.vas.df, method = "REML")

## Create a matrix of residuals
post_vas.resid <- cbind(right_0.vas.df[!(is.na(right_0.vas.df$percent_change)), c("subject_id", "frame")], m.post_vas$residuals)
...
pred_df <- data.frame(seconds = rep(seq(0, 400), 2)/30,
                      user = c(rep("not high", 401), rep("high", 401)), 
...
post_vasTraj <- grid.arrange(arrangeGrob(post_vasProfile+theme(legend.position = "none"), 
                                          post_vasProfile.se+theme(legend.position = "none"), nrow = 1), 
...
right.names <- c(right.names[1:3], paste0("R_", right.names[4:length(right.names)]))
names(right_0.post.w2) <- right.names

left_0.post.w2 <- left_0.post.w
left.names <- names(left_0.post.w2)
left.names <- c(left.names[1:3], paste0("L_", left.names[4:length(left.names)]))
...
right.corr.names <- right.names[4:length(right.names)]
left.corr.names <- left.names[4:length(left.names)]
...
right.names <- c(right.names[1:2], paste0("R_", right.names[3:length(right.names)]))
names(right_0.pt.w2) <- right.names

left_0.pt.w2 <- left_0.pt.w
left.names <- names(left_0.pt.w2)
left.names <- c(left.names[1:2], paste0("L_", left.names[3:length(left.names)]))
...
right.corr.names <- right.names[3:length(right.names)]
left.corr.names <- left.names[3:length(left.names)]

wpd_botheye.corr <- round(cor(wpd_botheyes[, right.corr.names[-1]],
                              wpd_botheyes[, left.corr.names[-1]], 
