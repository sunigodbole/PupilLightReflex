...
```{r auc_plot_post, fig.width=5, fig.height=5}
...
legend(0.55, 0.2, legend = c(paste0("Summary Features AUC: ",round(post.auc.scalar, 3)),
...
post_allProfile <- ggplot(data = pred_df_user, aes(x = seconds, y = mean, group = user, col = as.factor(user)))+
                   ...
                   scale_color_manual(values = c(user_col[c(2, 13, 11)]), 
                                      breaks = c("non-user", "occasional", "daily"))+
                   labs(y = "Percent Change in Pupil Response", 
...
ggplot(data= pt.analytic.df[pt.analytic.df$user_cat != "non-user", ], 
  ...
  scale_x_continuous(breaks = round(seq(53, 84, length.out = 5)),
                     labels = as.character(round(seq(53, 84, length.out = 5))))+
...
post_smokerProfile <- ggplot(data = pred_df, aes(x = seconds, y = mean, group = user, col = user))+
                    ...
                    scale_color_manual(values = c(user_col[c(2, 9, 11, 13, 15)]), 
                                      breaks = c("Non-Smoker", "Smoker (65 min)",
                                                 "Smoker (60 min)", "Smoker (70 min)"))+
                    theme_bw()+
                    theme(legend.position = c(0.9,0.2))
