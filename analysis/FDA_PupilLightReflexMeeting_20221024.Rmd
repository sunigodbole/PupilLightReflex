...
+ Functional Principal Components Analysis
    + variation explained mostly by minimal constriction and rebound shapes
+ Prediction Analysis:
    + AUC values range form 0.68 - 0.71
    + 1st 4 PCs from the fPCA analysis for within person difference data (WPD) have the higher prediction ability
+ Some association between demographic variables and PCs
+ Function on Scalar Regression (FoSR)
    + Significant differences between occasional and non-users in the minimal constriction time interval 
    + Significant differences between smokers and non-smokers in the minimal constriction time interval 
...
We'd examined the plot for the left and right eye and found them to be similar, so for this analysis we'll used data from the right eye and we'll explore the additional information provided by modeling both eye in the future.
...
I see some attenuation of the initial light reflex response between pre and post measures in most participants, but there is a lot of variability in the recovery trajectories (both duration and shape). We'll be exploring if these differences are significantly related to the participant's THC use category and if there might be differences by smoker vs non-smoker.
